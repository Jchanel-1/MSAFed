...
class MemContrastLoss(nn.Module, ABC):
    ...
    def _anchor_sampling(self,X,y_hat):
        """
        random sample anchor based on prediction for SSF
        """      
    ...
    def _sample_negative(self, Q): 
        ...
        if self.mode ==False:
            ...
            cache_size=self.args.num_cluster*5 
        feat_size=Q.memory[0][0].shape[0]
        X_ = torch.zeros((Q.get_size(), feat_size)).float().cuda()
        y_ = torch.zeros((Q.get_size(), 1)).float().cuda()
        sample_ptr = 0
        for ii in range(class_num):
            this_q =Q.get_class_elements(ii)
            X_[sample_ptr:sample_ptr+Q.get_class_size(ii),...]=this_q
            y_[sample_ptr:sample_ptr+Q.get_class_size(ii),...]=ii
            sample_ptr += Q.get_class_size(ii)
    ...
    def _contrastive(self, X_anchor, y_anchor, queue=None):
        ...
        log_prob=pos_logits-torch.log(pos_exp_logits+neg_exp_logits_sum)
        mean_log_prob=log_prob.sum(1)/pos_mask.sum(1)
        loss=-(self.temperature / self.base_temperature)*mean_log_prob
        loss=loss.mean()
        assert not torch.isnan(loss), 'Local memory loss is nan'
        return loss
    ...
    def forward(self, feats, labels=None, predict=None, queue=None):
        if self.args.pretrain: 
            ...
            assert not torch.isnan(feats).any(),"feats is nan"    
